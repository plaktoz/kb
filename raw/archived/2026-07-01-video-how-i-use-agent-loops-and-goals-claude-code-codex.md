...
**Owain Lewis:** This is a practical guide to using agent loops and goals in claw code and codeex. I'll show you the different ways I'm using these primitives in my own workflow as a professional AI developer.

If terms like loop engineering and agent loops feel confusing to you, it's not your fault. It's because people are using one vague term to describe many different things. So, in this video, we're going to break down how to use agent loops, goals, and scheduled automations to be more productive when building code with AI agents.

I'll also address the obvious criticisms when it comes to this topic, specifically around cost control and whether this is all just hype. All of the resources are going to be linked for free in the description below. So, let's get into it.
...
But we now have the goal primitive in both claw code and codeex. Goals are really powerful. I used to be an engineering manager and I think about this in the same way. There are two ways you can manage people in a team. The first way to manage people as a manager is to micromanage them. You can tell them what to do at every step.

You can say write this code, do this task, update this document. You're telling them what to do all of the time. The problem is that's not very good for people in general. No one likes being micromanaged. And it also doesn't scale very well because you have to constantly be in the loop as a manager telling everyone what to do all of the time. It just doesn't scale. And it's a really terrible way to manage people.

The other way you can manage is to give people a very clear goal to work towards and define what success looks like and let people work towards that. And I think that's a really good analogy for thinking about goals in general.

An important thing to know is that Claude code and Codex implements their goal implementation slightly differently. Codex uses a continuationbased pattern and Claude code uses some kind of LLM judge to decide whether the goal has been achieved. But they both basically work in the same way.

There are also two timebased features to think about. The first one in claude code is /loop. This is running something on an interval. So every 5 minutes do something. You can use this to maybe monitor something in the outside world. So like every 5 minutes go and check this pull request to see if there's any feedback. If there is, go and fix it.
...
**Owain Lewis:** All right, so let's take a look at some examples. So this is one of my favorite use cases for schedules in general. So what I have here is a codeex automation that's going to run every morning. The goal of this is to scan production logs and find errors basically. So find problems in production. And when you find a problem or a bug, reproduce the bug, fix it, add some tests, and then open a pull request.
...
So, let me quickly show you how this works. So, if we go over into the scheduled automations and we click down here, you can see there's a run that happened the other day. And what happened is the agent ran this automation. It found some problems in production and then it raised a pull request. And what you'll notice is these kind of things are very very difficult to detect otherwise.

You can see here we have a memory issue. So the cloud run web service was essentially using more memory than it should have done. So we need to tweak the memory settings on the container. This is a really really like subtle bug that you probably wouldn't notice otherwise. And what's really nice is it provides evidence as well. It actually tells you why it why it thinks that this is a problem. So this kind of thing is just really really powerful.


## Example: Deploying to Production with Goals [00:03:55]

**Owain Lewis:** Okay. So the next example I want to show you is again one of my favorite use cases for the goal feature is to work on a difficult longunning problem. So I was building a SAS application and what I wanted to do was deploy this to Google Cloud. So I had a SAS application that I've been building. It runs on my local machine, but it's not deployed to production.

So what I did is I set a goal to the agent to deploy this to production. This is quite a good goal for a couple of reasons. The main reason is that it's very easy for an agent to verify if it's achieved this goal. And it also involves doing lots and lots of tickets. I think there were like five or maybe even 10 tickets that the agent needed to work through in order to config configure this and actually do it.

The agent can use browser automation, and it can use any kind of different ways it wants to check if the application is live and healthy. So here it is on Google Cloud Run. This was an application running on my local machine that the agents deployed to production for me. I didn't do a thing.

So we gave it a very clear goal. Deploy the application to this new Google Cloud project. Um we also want to set up CI/CD automation. So basically what happens is whenever I push my code to GitHub then the uh the whole project will get auto deployed as well. So the agent did a ton of work here and this would have taken me maybe a couple of hours if I were to do this by hand.

So again, I think this is a really great use case for goals. So one of the criticisms you might have here is that it's risky to deploy or allow agents to do this much work unattended and it kind of makes sense if you're building a production application. I've worked on cloud systems that with, you know, 99.99% SLA uptime. I wouldn't do this in that situation.

But this is a personal project. The work is extremely well speced out. I spent a lot of time planning this work out. The agent knows exactly what it needs to do and mostly this work is just mechanical. There's not a lot of like human creativity or thinking needed to deploy this application. It's just running through the steps that I've already defined for it. So I think this is a good use case for goals. Obviously, this is just to show you what's possible. I'm not saying you should deploy your application with agents. It's kind of entirely up to you.

As you work on these goals, you can do whatever you want here. So you can do like multiple different check criteria. So for example, we can make sure that the the health endpoint returns okay, the uh the web application returns okay. You can kind of get creative here with how you want to have the agent confirm that the work is done correctly. But that's the beauty of goals. We're just really defining what we want to happen and how to measure success and allowing the agent to work within those constraints to figure out how to do it.


## Example: Working Through GitHub Issues [00:06:24]

**Owain Lewis:** All right. So another good use case I find for using goals is to just work through a bunch of issues. And this is a common thing where I've got a bunch of tasks I need to work through and I spent a lot of time specking these things out. They're kind of really minor things. It's worth saying this is a personal project. Like if this is critical stuff, you might not want to do this. Everything is kind of context dependent if that makes sense.

But I just want to work through these tickets. This is a proof of concept. I know all of these tickets really well and I just want to blast through them. So what I'm going to do is go into Claude code and then I'm going to paste in a prompt. And the prompt is going to say, "Work through all of the open issues. Start with any issues that are marked as ready. Implement one issue at a time. Run the tests. Open a pull request. Address any review feedback. And merge only when safe. You're done when every issue is closed, merged, or blocked with evidence."

So again, we're just allowing the agent to run through all of these different tickets. This would take me a very long time. If I was manually prompting the agent to like work on one ticket at a time, it would take me a long time. So I'm just using a goal here. I'm just going to let Claude code run through all of those tickets.


## Example: Issue Triage Automation [00:07:30]

**Owain Lewis:** So, another example that you can do is to triage issues. So, for example, this project here is a text editor that I'm building. So, if I was to run, I think it's cargo run. It's been a while.

So, this is a project I'm working on. It's a text editor. So, this is like a terminal based editor that I use to replace Vim or Emacs, things like that. So, it's just a text editor that I'm building. And this project is relatively complicated and it has a lot of tasks or tickets.

So what we can do is use claude code to triage all of the open tasks and add the correct labels and make sure everything is in a good state. So there's an example of a prompt you can do here. So in claude code you can say every Monday morning triage the GitHub issue backlog fix labels close duplicates with comments. Correct stale states and report anything that needs maintainer judgment.
...
The other one I use a lot is for triage. Not the most exciting use case in the world. Doesn't make for a great demo on YouTube, but I use this all of the time. So find problems in the code, tidy things up, make everything better.


## Conclusion [00:13:12]

**Owain Lewis:** If you want to go deeper on any of these topics, I run an AI engineering community where we have courses on agentic coding, building production AI systems, and also how to monetize your skills as a developer. I also offer one-on-one help and support inside the community as well. So, I'll put a link in the description below if you want to check it out.
